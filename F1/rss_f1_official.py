...
# KONFIGURACIJA
# ==========================================
# Službeni F1 RSS Feed kao početna točka
...
async def process_single_article(browser: Any, title: str, link: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        logging.info(f"🔍 Otvaram F1 članak: '{title[:30]}...'")
        await page.goto(link, wait_until="domcontentloaded", timeout=30000)
        
        soup = BeautifulSoup(await page.content(), 'html.parser')
        src = None
        
        # 1. Ciljamo Open Graph sliku (F1.com ima odlične og:image oznake visoke rezolucije)
        og_image = soup.find("meta", property="og:image") or soup.find("meta", name="twitter:image")
        if og_image and og_image.get("content"):
            src = og_image["content"]
            
        # 2. Fallback na JSON-LD ako og:image zakaže
        if not src:
...
async def scrape_f1_async() -> None:
    logging.info("🚀 Pokrećem F1 XML feed parsiranje i asinkrono izvlačenje slika...")
    
    try:
        # Prvo brzo povlačimo i parsiramo cijeli XML feed preko feedparser-a
        feed = feedparser.parse(START_URL)
        tasks_to_process = []
        
        for entry in feed.entries:
            title = entry.get('title', '')
            # Čistimo uobičajeni sufiks ako se pojavi u naslovu feeda
            title = re.sub(r'\s*-\s*Formula 1.*$', '', title, flags=re.IGNORECASE).strip()
            link = entry.get('link', '')
            
            if title and link:
                tasks_to_process.append((title, link))
                
        logging.info(f"📊 Detektirano {len(tasks_to_process)} vijesti unutar F1 XML feeda.")
        
        if not tasks_to_process:
            logging.warning("⚠️ XML feed je prazan ili nedostupan.")
            return

        # Pokrećemo Playwright samo za paralelno obradu pojedinačnih članaka
        async with async_playwright() as p:
            ...
            for i in range(0, len(tasks_to_process), CONCURRENT_TASKS):
                ...
                logging.info(f"📦 Otvaram paket od {len(chunk)} paralelna F1 članka...")
            ...
            if news_items:
                ...
                logging.info(f"🎉 Gotovo! 'F1/f1_official.json' uspješno spremljen s {len(news_items[:MAX_NEWS_ITEMS])} artikala.")
            
    except Exception as e:
        logging.error(f"❌ Kritična greška u glavnom F1 procesu: {e}")
